...
const set_message_timer = (message) => {
    ...
    // timer = setTimeout(realize_actions, 0, message)
    console.log(delay)
}
...
const message_processing = async (message) => {
    ...
    const need_add_message = await need_update(message.talk_id)
    if (!need_add_message) {
        return
    }
    ...
    const user_actions = await DB.find_actions(message.subdomain, {"manager":{$elemMatch:{"id":String(message.responsible_id)}}, "entity":'message'}) || []
    const group_actions = await DB.find_actions(message.subdomain, {"manager":{$elemMatch:{"id":`group_${message.group_id}`}}, "entity":'message'}) || []
    ...
    try{
        ...
        if (message.actions.notice){
            ...
            if (users[message.responsible_id]){
                users[message.responsible_id].write(`event: notice_message\ndata: ${message.actions.notice}\n\n`)
            }
        }
    } catch (error) {
